...
- [x] `UpdateChildDto` whitelist `name` + `sprite` uniquement (plus d'injection de champs)
- [x] expo-clipboard — import propre (suppression `@ts-ignore` + `require` dynamique)

### 🔲 À faire — Sécurité
- [ ] **IDOR complet** : vérifier toutes les routes parent/child (rewards, transactions, notifications) que `familyId`/`childId` JWT est contrôlé — P1
...
- [x] `expo-clipboard` installé et typé proprement
...
- [ ] Financial rails (Greenlight/BusyKid competitive) — v2
